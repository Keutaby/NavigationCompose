...
fun PantallaNavesEspaciales(modifier: Modifier, vm_swapi: SWAPIModelo){
    ...
    LaunchedEffect(Unit) { //Cuando la pantalla se redibuje
    ...
    Column(modifier = modifier){
        when(estado_de_swapi){
            ...
            Estados.mostrando_nave -> {
            }
            Estados.error -> {
                Text("Error, Parece que la informacion no esta disponible por el momento")
                //Text("Informacion tecnica: ${mensaje}") //mejor no mandar 404 a usuario
            ...
            Estados.registrar_frustracion -> {
                Text("Agitar tu telefonoo como rabieta no va a ayudar a resolver tu problema")
                Text("Tal vez agitarlo mas fuerte unnas 500 veces arregle el problema")
